  ...
  return (
    <aside className={`flex h-full min-h-0 shrink-0 select-none flex-col overflow-hidden border-r border-border bg-card transition-[width] duration-200 ${collapsed ? 'w-16' : 'w-72'}`} id="sidebar" data-collapsed={collapsed}>
      ...
      <Button
        ...
        className={collapsed ? 'mx-auto mt-2' : 'm-3'}
      ...
      <div className={`mt-auto shrink-0 border-t border-border ${collapsed ? 'p-1' : 'p-2'}`}><Button id="open-settings-btn" variant="ghost" size={collapsed ? 'icon' : 'default'} className={collapsed ? 'mx-auto' : 'w-full justify-start gap-2'} title="设置" onClick={() => setSettingsDialogOpen(true)}><Settings className="h-4 w-4" />{!collapsed && <span>设置</span>}</Button></div>
    </aside>
  );
}
